...
function renderBoard(){
  ...
  $("#board").innerHTML=shown.length?shown.map(p=>`<div class="row">
    ...
    <div class="pos">${p.pos}</div><div>${p.nfl}</div><div class="pickScore">${pickScore(p)}</div>
    <div>${p.espn_rank||"—"}</div><div class="sourceCount">${coverage(p)}/3</div>
  ...
  </div>`).join(""):'<div style="padding:28px;text-align:center;color:#7e8998;font-size:11px">No players match this view.</div>';
  $("#showing").textContent=state.view==="best"?`Top ${shown.length} by live Pick Score`:state.view==="targets"?`${shown.length} saved targets`:`Showing ${shown.length} of ${all.length} available`;
...
function wire(){
  ...
  $("#addBtn").addEventListener("click",()=>$("#playerDialog").showModal());
  $("#playerForm").addEventListener("submit",e=>{
    ...
    const id=`custom-${Date.now()}`;state.players.push({id,adp,name,nfl,pos:position,bye:null,owner:null,target:false,espn_rank:null,fantasypros_ecr:null});
...
async function init(){
  ...
  try{
    ...
    state.players=data.players.map((p,i)=>({...p,id:`p-${i}`,owner:null,target:false}));
